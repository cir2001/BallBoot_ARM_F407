...
//画笔颜色,背景颜色
u16 POINT_COLOR = 0x07E0;  
u16 BACK_COLOR = 0xFFFF;	//初始值为0xFFFF
...
void LCD_GPIOInit(void)
{
	...
	//GPIO_Set(GPIOB,PIN10|PIN11|PIN12|PIN13|PIN14|PIN15,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_50M,GPIO_PUPD_NONE);	//PB13~15复用功能输出	
	GPIO_Set(GPIOB,0xFC<<8,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_50M,GPIO_PUPD_NONE);	//PB13~15复用功能输出	
	delay_us(20);
}
//******************************************************************
//函数名：  LCD_Reset
...
void LCD_SetWindows(u16 xStar, u16 yStar,u16 xEnd,u16 yEnd)
{
	...
	LCD_WR_DATA(xStar>>8);
	LCD_WR_DATA(0x00FF&xStar+0);	//修改X起始坐标(左上角)		Star+0 表示坐标为0+0=0
	LCD_WR_DATA(xEnd>>8);
	LCD_WR_DATA(0x00FF&xEnd+2);		//修改X终点坐标(右下角)		end+0 表示坐标为0+0=0

	LCD_WR_REG(lcddev.setycmd);	
	LCD_WR_DATA(yStar>>8);
	LCD_WR_DATA(0x00FF&yStar+0);	//修改Y起始坐标(左上角)		Star+0 表示坐标为0+0=0	
	LCD_WR_DATA(yEnd>>8);
	LCD_WR_DATA(0x00FF&yEnd+2);		//修改Y起始坐标(左上角)		Star+0 表示坐标为0+0=0
...
{ 	
	...
	LCD_WriteReg(0x36,0x60); //镜像 横屏  RGB模式  不是BGR模式
