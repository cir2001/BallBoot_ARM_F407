...
//THUMB指令不支持汇编内联
//采用如下方法实现执行汇编指令WFI  
//Keil编译器不支持WFI指令,所以采用如下方法实现WFI指令
...
//GCC编译器支持WFI指令,所以直接使用CMSIS标准内置函数实现WFI指令 PlatformIO下修改
...
{
    __disable_irq(); // CMSIS 标准内置函数
}

// 开启所有中断
void INTX_ENABLE(void)
{
    __enable_irq(); // CMSIS 标准内置函数
}
